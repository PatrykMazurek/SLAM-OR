...
from OR.Yolo.yolov3.configs import *
from OR.Yolo.yolov3.yolov4 import *
from OR.Yolo.box import BoundBox

# YOLO_COCO_CLASSES ='/home/patryk/Git/pyslam/Yolo/model_data/coco/coco.names'

YOLO_CLASS_NAME = ["Car", "Van", "Truck", "Person", "Bus", "Cyclist", "Tram", "Traffic light", "Traffic sign"]
FIRST_LINE = True
...
def Convert_To(bboxes, b_format, frame_id):
    global FIRST_LINE
    list_box = list()
    with open("object_frame_test.txt", "a+") as frame:
        if FIRST_LINE:
            frame.write("id xmin ymin xmax ymax score class_ind color label\n")
            FIRST_LINE = False
        for i, box in enumerate(bboxes):
            ...
            color = [[1, 0, 0], [0, 1, 0], [0, 0, 1], [1, 0, 1], [0, 1, 1], [1, 1, 0],
                     [0.117647, 0.113725, 0.392156], [1, 0.615686, 0],
                     [0.419607, 0.317647, 0.074509]]
            bbox_color = color[class_ind]
            label = YOLO_CLASS_NAME[class_ind]
            # bbox_thick = int(0.6 * (image_h + image_w) / 1000)
            # if bbox_thick < 1: bbox_thick = 1
            # fontScale = 0.75 * bbox_thick
            if b_format == 'box':
                b = BoundBox(coor[0], coor[1], coor[2], coor[3])
                b.classes = class_ind
                b.score = score
                b.color = bbox_color
                b.v_label = label
                ...
                frame.write("{};{};{};{};{};{};{};{};{}\n".format(frame_id, int(coor[0]), int(coor[1]), int(coor[2]), int(coor[3]),
                        box[4], int(box[5]), bbox_color, label))
            elif b_format == 'json':
                obj = {"xmin": int(coor[0]), "ymin": int(coor[1]), "xmax": int(coor[2]), "ymax": int(coor[3]),
                       "score": box[4], "class_ind": int(box[5]), "color": bbox_color, "label": label}
                list_box.append(obj)
...
def detect_frame(Yolo, original_image, input_size, frame_id, score_threshold=0.3, iou_threshold=0.45, json_format=False):
    image_data = image_preprocess(np.copy(original_image), [input_size, input_size])
    image_data = image_data[np.newaxis, ...].astype(np.float32)
    ...
    # konwersja obiektów na odpowiednie formaty (box lub json)
    if json_format:
        # konwersja na json format
        return None
    else:
        return Convert_To(bboxes, 'box', frame_id)


def Predict_bbox_mp(yolo, Frames_data, Predicted_data, Processing_times, ):
    gpus = tf.config.experimental.list_physical_devices('GPU')
    if len(gpus) > 0:
        try: tf.config.experimental.set_memory_growth(gpus[0], True)
