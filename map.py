...
class Map(object):
    def __init__(self):
        ...
        # detecting point and color of point
        self.d_point = list()
        self.map_point = set()
        # self.map_point_color = set()
        # local map 
    ...
    # znajdz konkretny punkt i zaktualizuj jego kolor
    def update_point_color(self, point_set):
        with self._lock:
            self.points.update(point_set)

    def num_points(self):
        with self._lock: 
            return len(self.points)                

    def add_map_point(self, temp_point):
        with self._lock:
            self.map_point.add(temp_point)
    #
    # def add_map_point_color(self, temp_color):
    #     with self._lock:
    #         self.map_point_color.add(temp_color)
    #
    def get_all_detecting_point(self):
        with self._lock:
            return self.map_point.copy()

    def num_detecting_point(self):
        with self._lock:
            return len(self.map_point)
    ...
    def draw_feature_trails(self, img, list_box):
        if len(self.frames) > 0:
            img_draw = self.frames[-1].draw_all_feature_trails(img, list_box)
            return img_draw
        return img
