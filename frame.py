...
class Frame(FrameBase):
    ...
    def __init__(self, img, camera, pose=None, id=None, timestamp=None, kps_data=None):
        ...
        if img is not None:
            ...
            if kps_data is None:   
                ...
                #print("kps {}".format(type(self.kps)))
                #print("des {}".format(type(self.des)))
                # convert from a list of keypoints to arrays of points, octaves, sizes  
                self.temp_kps = kps_data = np.array([ [x.pt[0], x.pt[1], x.octave, x.size, x.angle] for x in self.kps ], dtype=np.float32)
                self.kps     = kps_data[:,:2]    
    ...
    def draw_feature_trails(self, img, kps_idxs, list_box, trail_max_length=9):
        img = img.copy()
        i = 0
        with self._lock_features:
            ...
            #print("wielkość pointd: {} kps: {}".format(len(self.points), kps_idxs))
            ...
            for kp_idx in kps_idxs:
                ...
                if point is not None and not point.is_bad:
                    ...
                    for box in list_box:
                        if box.xmin < uv[0] and box.xmax > uv[0] and box.ymin < uv[1] and box.ymax > uv[1]:
                            #print("{}".format((box.color[0]*255, box.color[1]*255, box.color[2]*255)))
                            col = np.array(box.color) * 255
                            cv2.circle(img, uv, color=(int(col[2]), int(col[1]), int(col[0])), radius=5, thickness=1)
                            # print(col)
                            cv2.rectangle(img, (box.xmin, box.ymin), (box.xmax, box.ymax), (int(col[2]), int(col[1]), int(col[0])), thickness=2)
                            cv2.putText(img, box.v_label, (box.xmin, box.ymin), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                                        1, (int(col[2]), int(col[1]), int(col[0])), 1)

                            #print("Dopasowano punkt")
                            #print("box: {} {} {} {} point {}".format(box.xmin, box.xmax, box.ymin, box.ymax, uv))
                        #cv2.circle(img, uv, color=(0, 255, 0), radius=1, thickness=1)  # draw keypoint size as a circle
                    #    #cv2.circle(img, uv, color=color, radius=radius)
                    #else:
                    #    cv2.circle(img, uv, color=(255, 0, 0), radius=1, thickness=1)  # draw keypoint size as a circle
                ...
                #     cv2.circle(img, uv, color=(0, 0, 0), radius=1) #radius=radius)
            #print("Liczba poprawnych punktów {}".format(i))
            return img
        
    # draw tracked features on the image
    def draw_all_feature_trails(self, img, list_box):
        kps_idxs = range(len(self.kps))
        return self.draw_feature_trails(img, kps_idxs, list_box)
