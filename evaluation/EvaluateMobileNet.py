...
for bb in range(len(arr_dbscan_eps)):
    for cc in range(len(arr_dbscan_min)):
        for aa in range(len(arr_recording_id)):
            ...
            fig_ref = "resultsMobileNet/reference_id=" + recording_id + ".png"
            fig_res = "resultsMobileNet/results_id=" + recording_id + ",eps=" + str(dbscan_eps) + ",min=" + str(dbscan_min) + ",box_size=1.75.png"
            ...
            with open('resultsMobileNet/resultsMobileNet.txt', 'a') as the_file:
