...
for cc in range(len(arr_dbscan_min)):
    for aa in range(len(arr_recording_id)):
        ...
        fig_res = "results/optics_results_id=" + recording_id + ",min=" + str(dbscan_min) + ",box_size=1.75.png"
        ...
        my_str = recording_id + "\t" + str(dbscan_min) + "\t"  + str(TPR) + "\t" + str(PPV) + "\t" + str(FNR) + "\t" + str(F1) + "\t" + str(FM)
        print(my_str)

        with open('results/results_optics.txt', 'a') as the_file:
