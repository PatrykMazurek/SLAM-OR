...
class Kitti:
    ...
    def __init__(self, frame='all', velo_path=None, camera_path=None, \
                 img_type='gray', v2c_path=None, c2c_path=None, xml_path=None, yaw_correction = None):
    ...
    def __load_tracklet(self):
        ...
        for tracklet in tracklets:
            ...
            help_iter = 0
            my_size = len(tracklet.trans) - 1
            translation = tracklet.trans[my_size]
            rotation = tracklet.rots[my_size]
            state = tracklet.states[my_size]
            occlusion = tracklet.occs[my_size]
            truncation = tracklet.truncs[my_size]
            amtOcclusion = tracklet.amtOccs[my_size]
            amtBorders = tracklet.amtBorders[my_size]
            absoluteFrameNumber = range(tracklet.firstFrame, tracklet.firstFrame + tracklet.nFrames)
            ...
            for translation, rotation, state, occlusion, truncation, amtOcclusion, amtBorders, absoluteFrameNumber in tracklet:
                #tutututu
                #print(tracklet.firstFrame + tracklet.nFrames  - 2)
                #if tracklet.firstFrame == absoluteFrameNumber:
                #help_iter = help_iter + 1
                #if help_iter == tracklet.nFrames:
                #if tracklet.firstFrame + tracklet.nFrames - 2 == absoluteFrameNumber:
                #if True:
                iter = iter + 1
                if iter == tracklet.nFrames:
                #if tracklet.isFinished == True:
                    ...
                    #yaw = yaw + 0.8222153
                    if self.yaw_correction is not None:
                        a = 0
                        #print(yaw)
                        #yaw = yaw + (-3.14 / 2.0) + self.yaw_correction
                        #print(yaw)
                    rotMat = np.array([ \
                    ...
                    if True:
                        #yaw = (3 * 3.14/2.0)+self.yaw_correction
                        #yaw = (3.14 / 2.0) - self.yaw_correction
                        ...
                        rotMat = np.array([ \
                            ...
                            [np.sin(yaw), np.cos(yaw), 0.0], \
                            [0.0, 0.0, 1.0]])
                        cornerPosInVelo = np.dot(rotMat, cornerPosInVelo)
...
class Kitti_util(Kitti):
    ...
    def __init__(self, frame='all', velo_path=None, camera_path=None, \
                 img_type='gray', v2c_path=None, c2c_path=None, xml_path=None, yaw_correction = None):
        super(Kitti_util, self).__init__(frame, velo_path, camera_path, img_type, v2c_path, c2c_path, xml_path, yaw_correction)
