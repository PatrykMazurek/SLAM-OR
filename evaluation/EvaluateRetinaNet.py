...
for bb in range(len(arr_dbscan_eps)):
    for cc in range(len(arr_dbscan_min)):
        for aa in range(len(arr_recording_id)):
            ...
            fig_ref = "resultsRetinaNet/reference_id=" + recording_id + ".png"
            fig_res = "resultsRetinaNet/results_id=" + recording_id + ",eps=" + str(dbscan_eps) + ",min=" + str(dbscan_min) + ",box_size=1.75.png"
            ...
            with open('resultsRetinaNet/resultsRetinaNet.txt', 'a') as the_file:
